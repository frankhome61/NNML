    ...
    def backprop(self):
        ...
        for n in self.nodes[-1::-1]:
            if isinstance(n, FunctionNode):
                input = self.get_inputs(n)
                pre_gradient = self.get_gradient(n)
                parents = n.get_parents()
                curr_gradients = n.backward(input, pre_gradient)
                for parent_node, curr_gradient in zip(parents, curr_gradients):
                    self.gradients[parent_node] += curr_gradient
    ...
    def step(self, step_size):
        ...
        for n in self.nodes:
            if isinstance(n, Variable):
                node_data = n.data
                gradient = self.gradients[n]
                node_data -= step_size * gradient
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        A = inputs[0]
        x = inputs[1]
        gradient_A = gradient * np.ones_like(A)
        gradient_x = np.dot(np.ones_like(A).T, gradient)
        return [gradient_A, gradient_x[0]]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        A = inputs[0]
        B = inputs[1]
        batch_size, dim = np.shape(A)
        gradient_A = (A - B) / (batch_size * dim)
        gradient_B = (B - A) / (batch_size * dim)
        return [np.dot(gradient, gradient_A), np.dot(gradient, gradient_B)]
